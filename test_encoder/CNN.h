...
void CNN( int padding, int width, int hight,  hls::stream<double> &image, hls::stream<data_t> &output,hls::stream<data_t> &output_pooling_1);
...
template <typename INPUT_DATA_TYPE
	 ...
	 >
void convolution( int padding, int width, int hight ,INPUT_DATA_TYPE bias[CONV_NUM_CHANNEL], hls::stream<INPUT_DATA_TYPE> &image, KERNEL_TYPE kernel[CONV_NUM_KERNEL][CONV_NUM_CHANNEL][KERNEL_XSIZE][KERNEL_YSIZE],hls::stream<OUTPUT_DATA_TYPE> &output) {
...
Shift_win_right:for( int b = 0 ; b < width - 2 ; b++) {
      ...
      for( int num_ker= 0 ; num_ker < CONV_NUM_KERNEL ; num_ker ++){
	      for( int num_chan; num_chan < CONV_NUM_CHANNEL ; num_chan++){
INPUT_DATA_TYPE sum = 0;
          for (int m = 0; m < KERNEL_XSIZE; m++) {
             for (int n = 0; n < KERNEL_YSIZE; n++) {
             sum += window_buffer[m][n] * kernel[num_ker][num_chan][m][n];
            }
	  }
	  sum += bias[num_ker];
          output << relu(sum);
